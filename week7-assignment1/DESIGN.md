...
**Cached read: a project's task list** (`GET /api/v1/projects/:id/tasks?page=1&status=todo`).
...
**Read that must NOT be cached:** `GET /api/v1/projects/:id/members` (the membership list). Role checks depend on this data being current — a cached membership list could let a recently removed user appear authorised. The membership list is small (tens of rows) and queried less often than the task list, so the cost of always hitting the database is negligible.

### 4.2 Scaling

**Read/write split.** The primary PostgreSQL instance handles all writes. A streaming replica handles read-only queries — all list endpoints (`GET .../tasks`, `GET .../comments`, etc.) are routed to the replica by a read-only connection pool in the repository layer. Writes (`INSERT`, `UPDATE`, `DELETE`) go to the primary.
...
**Criterion:** the task list UI uses infinite scroll, not numbered page controls. Users scroll forward through a filtered, sorted backlog; they never need to jump to page 47. The read pattern is sequential.

**Decision: cursor pagination** for all task list endpoints. Offset pagination is available as a fallback for admin/reporting views where the dataset is small and page-number navigation is useful.
